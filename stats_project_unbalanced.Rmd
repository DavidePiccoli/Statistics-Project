...
set.seed(13)  # to keep same values for later runs
...
random_probs1 <- runif(length(answers))
random_probs2 <- runif(length(answers))
...
n_hurricane <- 10000
answers_hurricane <- sample(answers, n_hurricane, replace = TRUE, prob = random_probs1)
```

```{r}
# Generate answers for those who did not witness a natural disaster
n_no_hurricane <- 10000
answers_no_hurricane <- sample(answers, n_no_hurricane, replace = TRUE, prob = random_probs2)
...
ks_test <- ks.test(cdf_hurricane, cdf_no_hurricane)
print(ks_test)
```
```{r}
# Chi-square test
chi_square_test <- chisq.test(cdf_hurricane, cdf_no_hurricane)
...
ks_test_skewed <- ks.test(cdf_hurricane_skewed, cdf_no_hurricane_skewed)
print(ks_test_skewed)
```
```{r}
# Chi-square test on skewed distributions
chi_square_test_skewed <- chisq.test(cdf_hurricane_skewed, cdf_no_hurricane_skewed)
