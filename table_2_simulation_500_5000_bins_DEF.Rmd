...
# Perform chi-square test
#chi_sq_result <- chisq.test(x_bin, y_bin, p=c(0.25, 0.25, 0.25, 0.25))    # testing p
chi_sq_result <- chisq.test(table)
...
x_bin <- ifelse(y_i <= 2, 1,
               ifelse(y_i > 2 & y_i <= 3, 2,
                      ifelse(y_i > 3 & y_i <= 4, 3, 4)))

# Define the four-category variable
y_bin <- ifelse(y_i_2 <= 2, 1,
...
# Perform chi-square test
chi_sq_result <- chisq.test(table, simulate.p.value = TRUE, B = 1000)
